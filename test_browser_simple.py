...
def http_get(url: str, timeout: float = 5.0) -> tuple[int, str]:
    """Basic HTTP GET helper to avoid external dependencies."""
    try:
...
def test_server_and_pages():
    ...
    try:
        ...
        try:
            status, _ = http_get("http://localhost:8000/")
            test_results.append(("Server running", "PASSED" if status == 200 else f"FAILED: {status}"))
        except Exception as e:
            test_results.append(("Server running", f"FAILED: {str(e)}"))
        
        # Test 2: Index page loads
        try:
            status, content = http_get("http://localhost:8000/index.html")
            test_results.append(("Index page loads", "PASSED" if status == 200 else f"FAILED: {status}"))
            ...
            if status == 200:
                ...
                has_nav = ("nav-list" in content) or ("tree-nav" in content)
        ...
        try:
            status, content = http_get("http://localhost:8000/chapter1.html")
            test_results.append(("Chapter1 page loads", "PASSED" if status == 200 else f"FAILED: {status}"))
        ...
        for asset in ["assets/style.css", "assets/script.js", "assets/highlight.css"]:
            try:
                status, _ = http_get(f"http://localhost:8000/{asset}")
                test_results.append((f"{asset} loads", "PASSED" if status == 200 else f"FAILED: {status}"))
            except Exception as e:
                test_results.append((f"{asset} loads", f"FAILED: {str(e)}"))
        
        # Test 5: Check links in content
        try:
            _, content = http_get("http://localhost:8000/index.html")
